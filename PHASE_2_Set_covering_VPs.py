...
filename = "00_Output Collection/output 221014-080250.json"
...
ph = "PHASE 2 STEP 1:"
des = "Set Covering get minimum required VPs"
#______________________________________________________________________________________
print("\n \n \n============================================== \n", ph, "\n \n", des)
...
for _vpKey in list(dataCollection["VPs"]):
    if len(dataCollection["VPs"][_vpKey]["visible Obj Voxel Indices"]) > 10:
        subsets.append(set(dataCollection["VPs"][_vpKey]["visible Obj Voxel Indices"]))
...
dataCollection["statistics"]["VP_setCovering"]={
    "Global Coverage (%)" : (_coveredObjVxls_unique/dataCollection["statistics"]["Occupied Object Voxel Count"])*100
}

_allVPkeys_before = list(dataCollection["VPs"].keys())
...
for res in scpResults:
    newKey = "VPs_"+res
    dataCollection[newKey] = {}
    for i in scpResults[res]["idx"]:
        dataCollection[newKey][_allVPkeys_before[i]] = copy.deepcopy(dataCollection["VPs"][_allVPkeys_before[i]])
    
    resultFitness = scp.getFittness(universe, subsets, scpResults[res]["enc"])

    dataCollection["statistics"]["VP_setCovering"][res] = {}

    dataCollection["statistics"]["VP_setCovering"][res]["FitnessScore"] = resultFitness[0]
    dataCollection["statistics"]["VP_setCovering"][res]["Not Covered Count"] = resultFitness[1]
    dataCollection["statistics"]["VP_setCovering"][res]["Final ViewPoint Count"] = resultFitness[2]
    dataCollection["statistics"]["VP_setCovering"][res]["Unique Coverage Overlap (%)"] = resultFitness[3]
    dataCollection["statistics"]["VP_setCovering"][res]["Stacked Coverage Overlap (%)"] = resultFitness[4]
    dataCollection["statistics"]["VP_setCovering"][res]["Execution Time (s)"] = scpResults[res]["execTime"]
...
# Save statistics
dataCollection["statistics"]["execTimes"]["PHASE 2 STEP 1: Set Covering VPs - TIME (s)"] = execTime
...
del dataCollection["VPs"]


now = datetime.now()
now_str = now.strftime("%y%m%d-%H%M%S")

filename = "00_Output Collection/PH2 output " + now_str +".json"
